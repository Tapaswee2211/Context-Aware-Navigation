# smart_nav_vision2.py
# Location: ~/clearpath_ws/src/context_aware_nav/context_aware_nav/smart_nav_vision2.py
#
# ╔══════════════════════════════════════════════════════════════════════════╗
...
class SmartNavNode(Node):
    ...
    def __init__(self):
        ...
        # In SmartNavNode.__init__(), after other subscriptions:
        self.create_subscription(
            String, '/smart_nav/context',
            self._context_cb, 10)

        # Dynamic parameters (overridden by context manager)
        self._dyn_speed_limit      = FORWARD_SPEED
        self._dyn_turn_commit_secs = TURN_COMMIT_SECS

        # ── Added Status Publisher for markers ───────────────────────────────────────────────────
        # Status for markers
        self.status_pub = self.create_publisher(
                String, '/smart_nav/status', 10)
        ...
        self.grasp_trigger_pub = self.create_publisher(
            Bool, '/smart_nav/trigger_grasp', 10)
        self.create_subscription(
            String, '/smart_nav/grasp_result',
            self._grasp_result_cb, 10)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    #  Status Publisher for markers 
    # ─────────────────────────────────────────────────────────────────────────
    def _publish_status(self):
        """Publish current nav status for marker node and monitoring."""
        msg = String()
        msg.data = json.dumps({
            "waypoint_idx": self.waypoint_idx,
            "visited":      list(range(self.waypoint_idx)),
            "state":        self.state,
            "pos":          [round(self.robot_x, 2), round(self.robot_y, 2)],
        })
    ...
    def _context_cb(self, msg):
        try:
            ...
            ctx  = data.get("context", "OPEN")
            # Dynamically adjust speed and turn commitment
            self._dyn_speed_limit      = data.get("speed_limit",      FORWARD_SPEED)
            self._dyn_turn_commit_secs = data.get("turn_commit_secs", TURN_COMMIT_SECS)
            self.get_logger().info(
                f"Context update: {ctx} | "
                f"speed={self._dyn_speed_limit:.2f} "
                f"commit={self._dyn_turn_commit_secs:.1f}s")
        except Exception as e:
            self.get_logger().error(f"Context parse error: {e}")
    ...
    def _scan_cb(self, msg):
        ...
        if self.state == S.TURN_COMMIT:
            ...
            if elapsed < self._dyn_turn_commit_secs and min_all > EMERGENCY_DIST + 0.1:
        ...
        if min_all > OBSTACLE_CAUTION:
            ...
            speed = FORWARD_SPEED_SLOW if self.yolo_slow else self._dyn_speed_limit
    ...
    def _run_waypoint_sequence(self):
        ...
        # In _run_waypoint_sequence(), add just before _send_trajectory call:
        # Trigger grasp node to attempt object pickup
        grasp_msg = Bool()
        grasp_msg.data = True
        self.grasp_trigger_pub.publish(grasp_msg)
